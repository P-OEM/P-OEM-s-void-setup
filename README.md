...
## [Thunderbird setting up rss](#setting-up-rss)
## [Thunderbird importing rss](#importing-rss)
...
## Setting up rss
* to set up rss, start by making an rss profile
    * go to top right burger menu
        * click on `New Account`
            * click on `Feed`
                * type in the account name you want
                    * then finish
* now click on the newly made account in the left menu
    * locate and click `manage Feed Subscriptions`
        * add rss url to `Feed URL` field
            * then click the `Add` button (remember that many url's need https:// in front of them to work, despite the page containing them might not having it)
## Importing rss
* to import already existing rss to your installation, you need to locate the `Feeds` folder
* open your file manager, and navigate to your users home folder
    * then hit `ctrl` + `h` to unhide all `.` files
        * then click on `.thunderbird`, and go to `.thunderbird/oneOfTheRandomSymbolsFolders.default/Mail`
            * then replace the `Feeds` folder
                * my answer here too is making a shortcut. I make one to my `Feeds` folder in dropbox, and place it in Thunderbird's `Mail` folder
